...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      // For now, create mock top interactions (in real app, you'd analyze actual interactions)
      const topInteractions = followers.slice(0, 8).map((follower: any, index: number) => ({
        fid: follower.fid,
